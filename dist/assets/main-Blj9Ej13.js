(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function s(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=s(n);fetch(n.href,i)}})();let S=null;async function Z(){var e;if(S)return S;try{const t=await fetch("/data/profile_data/profile.json");if(!t.ok)throw new Error(`HTTP ${t.status}`);return S=await t.json(),console.log("✅ Profile loaded:",(e=S.personal)==null?void 0:e.name),S}catch(t){return console.error("❌ Failed to load profile:",t),null}}function Q(){return S}const q={};function j(e,t){q[e.toLowerCase()]=t}function ee(e){return q[e.toLowerCase()]}function te(e){window.location.hash=`#/${e}`}function V(){return(window.location.hash.slice(1)||"/home").replace(/^\//,"").toLowerCase()}const ae=[{label:"Home",path:"home"},{label:"Research",path:"research"},{label:"Publications",path:"publications"},{label:"Academics",path:"academics"},{label:"Experience",path:"experience"},{label:"Projects",path:"projects"},{label:"Hobbies",path:"hobbies"},{label:"Contact",path:"contact"}];let w="home";function se(e){var a;const t=((a=e==null?void 0:e.personal)==null?void 0:a.nickname)||"YUSUF_SILICON";w=V();const s=ae.map(n=>{const i=n.path===w;return`
        <a href="#/${n.path}"
  ...
  `}function ce(){const e=document.getElementById("theme-toggle-btn");e&&e.addEventListener("click",()=>{it(),J()})}function J(){const e=document.getElementById("theme-toggle-btn");if(!e)return;const t=e.querySelector(".material-symbols-outlined");if(!t)return;const s=document.documentElement.classList.contains("dark");t.textContent=s?"dark_mode":"light_mode"}function O(e){return e?e.replace(/^\.\.\//,"/data/"):""}function re(e,t){const s=(e==null?void 0:e.name)||"",a=(e==null?void 0:e.headline)||"",n=(e==null?void 0:e.description)||"",i=(e==null?void 0:e.description_highlight)||[],o=O(e==null?void 0:e.image),r=((t==null?void 0:t.description_mentions)||[]).map(d=>`<span class="chip">${d.toUpperCase().replace(/\s+/g,"_")}</span>`).join("");let l=n;if(i.length){const d=[...i].sort((u,p)=>p.length-u.length);for(const u of d){const p=u.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");l=l.replace(new RegExp(`(${p})`,"gi"),'<span class="home-highlight">$1</span>')}}return`
  ...
  `}function Se(e){const t=(e||[]).filter(a=>a.status!=="active");return t.length?`
    <section class="content-section research-section">
  ...
  `}j("research",{render:je});function Ie(e,t){return`
    <div class="research-annotation">
      <span class="research-annotation-badge font-mono-data">PAGE. ${e}</span>
      <div class="research-annotation-line"></div>
      <span class="font-label-caps research-annotation-text">[DOCUMENT_CLASS: ${t}]</span>
  ...
  `}function xe(e){if(!e||!e.length)return`
      <div class="content-section">
  ...
  `}function Re(e){var s;if(!e)return`<div class="container-page" style="padding-top: 4rem;">
      <p class="font-mono-data" style="color: var(--error);">No profile data available.</p>
  ...
  `,document.body.appendChild(s),requestAnimationFrame(()=>s.classList.add("proj-gallery-overlay--open")),s.querySelector(".proj-gallery-close").addEventListener("click",()=>B(s)),s.querySelector(".proj-gallery-backdrop").addEventListener("click",()=>B(s))}function B(e){e.classList.remove("proj-gallery-overlay--open"),e.addEventListener("transitionend",()=>e.remove(),{once:!0})}const tt=k;k=function(e){const t=Y(e==null?void 0:e.projects);return window.__projProjects=t,tt(e)};j("projects",{render:k,mount:W});const at=document.getElementById("app");function st(){at.innerHTML=`
    <header id="app-header"></header>
    <main id="page-content"></main>
    <footer id="app-footer"></footer>
  `}async function F(){const e=V(),t=Q(),s=ee(e),a=document.getElementById("page-content");if(a){if(s&&s.render){const n=await s.render(t);a.innerHTML=n,s.mount&&s.mount()}else e==="home"?a.innerHTML='<div class="container-page" style="padding-top: 4rem; padding-bottom: 4rem;"><p class="font-mono-data" style="color: var(--on-surface-variant);">// PAGE: HOME — Coming in Stage C</p></div>':a.innerHTML=`<div class="container-page" style="padding-top: 4rem; padding-bottom: 4rem;"><p class="font-mono-data" style="color: var(--on-surface-variant);">// PAGE: ${e.toUpperCase()} — Under construction</p></div>`;window.__updateActiveNav&&window.__updateActiveNav(e)}}function nt(){const e=localStorage.getItem("theme"),t=window.matchMedia("(prefers-color-scheme: dark)").matches;e==="dark"||!e&&t?document.documentElement.classList.add("dark"):document.documentElement.classList.remove("dark")}function it(){document.documentElement.classList.toggle("dark");const e=document.documentElement.classList.contains("dark");localStorage.setItem("theme",e?"dark":"light"),J()}async function ot(){nt(),st();const e=await Z();if(!e){const n=document.getElementById("page-content");n&&(n.innerHTML='<p style="padding: 2rem; color: var(--error);">Failed to load profile data.</p>');return}const t=document.getElementById("app-header");t&&(t.innerHTML=se(e),ne());const s=document.getElementById("theme-toggle-slot");s&&(s.innerHTML=oe(),ce());const a=document.getElementById("app-footer");a&&(a.innerHTML=ie(e)),he(),z(),W(),window.addEventListener("hashchange",F),F(),console.log("🔧 Yusuf Silicon Portfolio — Ready")}ot().catch(e=>console.error("❌ App initialization failed:",e));
